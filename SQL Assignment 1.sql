...
-- Modify "designation" to support wider range of values
ALTER Table employees
Modify column designation varchar(260);
...
-- Distict value
-- Retrieve distict salaries from the Employees table
...
-- ALIAS
-- Alias "age" and "salary" as "Employee_Age" and "Employee_Salary"
SELECT employee_id,employee_name,age as Employee_Age,salary as Employee_Salary from employees;
...
-- INNER JOIN
-- Employee names, designations, and department names (only where department is assigned)
...
-- RIGHT JOIN
-- All locations with employee names; NULL if no employee assigned
...
JOIN employees e2
   ...
   AND e1.employee_id < e2.employee_id   -- avoids self-pairs & duplicate reverse pairs
